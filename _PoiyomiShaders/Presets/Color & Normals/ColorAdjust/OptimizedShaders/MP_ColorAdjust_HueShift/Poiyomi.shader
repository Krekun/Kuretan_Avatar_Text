Shader "Hidden/Locked/.poiyomi/Poiyomi 8.0/Poiyomi Toon/80e5451b6d4c5d147abcb49b5a25cae9"
{
	Properties
	{
 ...
 #define PROP_FLIPBOOKTEXARRAY 
			...
			void applyFlipbook(inout PoiFragData poiFragData, in PoiMesh poiMesh, in PoiMods poiMods)
			{
				...
				float flipBookMask = POI2D_SAMPLER_PAN(_FlipbookMask, _MainTex, poiMesh.uv[float(0)], float4(0,0,0,0)).r;
				#else
				...
				if (float(0))
				{
					totalFrames -= (totalFrames - min(max(float(0), float(0)), totalFrames));
					totalFrames -= max(0, float(0));
				}
				if (!float(0))
				{
					if (float(6) != 0)
					{
						currentFrame = (_Time.y / (1 / float(6))) % totalFrames;
						if (float(0))
						{
				...
				{
					currentFrame = fmod(float(-1), totalFrames);
				}
				#ifdef POI_AUDIOLINK
				if (poiMods.audioLinkAvailable)
				{
				...
				if (float(0))
				{
					flipBookPixel.rgb = poiThemeColor(poiMods, float4(0.957614,0.957614,0.957614,1).rgb, float(0));
				}
				else
				{
					flipBookPixel.rgb *= poiThemeColor(poiMods, float4(0.957614,0.957614,0.957614,1).rgb, float(0));
				...
				if (poiMods.audioLinkAvailable)
				{
					flipbookAlpha += saturate(lerp(float4(0,0,0,0).x, float4(0,0,0,0).y, poiMods.audioLink[float(0)]));
				}
				#endif
				#if !defined(POI_PASS_OUTLINE) && !defined(UNITY_PASS_SHADOWCASTER)
				poiFragData.baseColor = lerp(poiFragData.baseColor, flipBookPixel.rgb, flipBookPixel.a * float4(0.957614,0.957614,0.957614,1).a * float(1) * flipBookMask * flipbookAlpha);
				poiFragData.baseColor = poiFragData.baseColor + flipBookPixel.rgb * float(0) * flipBookMask * flipbookAlpha;
				poiFragData.baseColor = poiFragData.baseColor * lerp(1, flipBookPixel.rgb, flipBookPixel.a * float4(0.957614,0.957614,0.957614,1).a * flipBookMask * float(0) * flipbookAlpha);
				...
				#endif
				poiFragData.emission += lerp(0, flipBookPixel.rgb * flipbookEmissionStrength, flipBookPixel.a * float4(0.957614,0.957614,0.957614,1).a * flipBookMask * flipbookAlpha);
				#endif
				
				if (float(0))
				{
					poiFragData.alpha = lerp(poiFragData.alpha, flipBookPixel.a * float4(0.957614,0.957614,0.957614,1).a, flipBookMask);
				}
				#endif
			}
			#endif
			float4 frag(v2f i, uint facing : SV_IsFrontFace) : SV_Target
			{
				...
				float4 mainTexture = UNITY_SAMPLE_TEX2D(_MainTex, poiUV(mainUV, float4(0,0,0,0)) + _Time.x * float4(0,0,0,0));
				...
				poiLight.lightMap = 1;
				#endif
				poiFragData.baseColor = mainTexture.rgb * poiThemeColor(poiMods, float4(0,0.01764195,0.021219,0).rgb, float(0));
				poiFragData.alpha = mainTexture.a * float4(0,0.01764195,0.021219,0).a;
				...
				clip(poiFragData.alpha - float(0.323));
