...
namespace Kuretan.AvatarTextTools.Utility
{
	public static class KatAnimatorInstaller
	{
		public static bool InstallToAnimator(AnimatorController controller, KatSettings settings)
		{
			...
			controller.AddParameter(settings.ParamTextLoading, AnimatorControllerParameterType.Bool);
			...
			controller.AddLayer(CreateLoadingLayer(controller, settings, animations));
		...
		public static bool RemoveFromAnimator(AnimatorController controller, KatSettings settings) {
			...
			for (int i = controller.layers.Length - 1; i >= 0; i--) {
				...
				if (
					...
					layer.name.StartsWith(settings.ParamTextLoading) ||
			...
			for (int i = controller.parameters.Length - 1; i >= 0; i--) {
				...
				if (
					...
					parameter.name.StartsWith(settings.ParamTextLoading) ||
		...
		private static AnimatorControllerLayer CreateLoadingLayer(AnimatorController controller, KatSettings settings, KatAnimations animations)
		{
			AnimatorControllerLayer layer = new AnimatorControllerLayer();
			AnimatorStateMachine stateMachine;

			layer.name = settings.ParamTextLoading;
			layer.defaultWeight = 1f;
			layer.stateMachine = stateMachine = new AnimatorStateMachine();
			layer.stateMachine.name = layer.name;
			AssetDatabase.AddObjectToAsset(layer.stateMachine, AssetDatabase.GetAssetPath(controller));
			...
			// Hide KAT state
			AnimatorState stateDisabled = CreateState(stateMachine, "Disabled", new Vector3(300f, 200f, 0f), animations.loadEnd);

			// Show KAT state
			AnimatorState stateEnabled = CreateState(stateMachine, "Enabled", new Vector3(300f, 300f, 0f), animations.loadStart);
			AnimatorStateTransition transitionEnable = CreateTransition(stateDisabled, stateEnabled);
			transitionEnable.AddCondition(AnimatorConditionMode.If, 0, settings.ParamTextLoading);
			AnimatorStateTransition transitionDisable = CreateTransition(stateEnabled, stateDisabled);
			transitionDisable.AddCondition(AnimatorConditionMode.IfNot, 0, settings.ParamTextLoading);
	...
	public class KatAnimations
	{
		...
		public readonly AnimationClip loadStart;
		public readonly AnimationClip loadEnd;
		...
		public KatAnimations(KatSettings settings)
		{
			...
			loadStart = Resources.Load<AnimationClip>(settings.CharacterAnimationFolder + "KAT_LoadStart");
			loadEnd = Resources.Load<AnimationClip>(settings.CharacterAnimationFolder + "KAT_LoadEnd");
